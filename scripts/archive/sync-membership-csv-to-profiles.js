...
function computeMembershipByUserId(csvPath) {
  ...
  if (
    atStatus !== 'active' &&
    atStatus !== 'expired' &&
    (firstFields[statusIdx + 1] || '').toLowerCase().trim() === 'active'
  ) {
    shift = 1;
  }
  ...
  if (
    expColIdx >= 0 &&
    !(firstFields[expColIdx] || '').trim() &&
    parseDateOnly((firstFields[expColIdx + 1] || '').trim())
  ) {
    expColIdx += 1;
  }
  const planNameIdx =
    planIdx >= 0 &&
    (firstFields[planIdx] || '').trim() === '' &&
    (firstFields[planIdx + 1] || '').trim() !== ''
      ? planIdx + 1
      : planIdx;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (
      !expStr &&
      expColIdx >= 0 &&
      fields[expColIdx + 1] != null &&
      parseDateOnly((fields[expColIdx + 1] || '').trim())
    ) {
    ...
    if (isActive) {
      ...
      if (since && (!entry.earliestSince || since < entry.earliestSince))
        entry.earliestSince = since;
...
async function main() {
  ...
  if (!existsSync(csvPath)) {
    ...
    console.error(
      'Usage: node scripts/sync-membership-csv-to-profiles.js [path-to-memberships.csv]'
    );
    process.exit(1);
  }
  ...
  for (const profile of profiles) {
    ...
    const membership_expiration =
      hasActive && entry?.membership_expiration ? entry.membership_expiration : null;
    ...
    const dateStr =
      member_since || membership_expiration
        ? ` since=${member_since ?? '—'} exp=${membership_expiration ?? '—'}`
        : '';
    console.log(
      '  wp_user_id',
      wpUserId,
