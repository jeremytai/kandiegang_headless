      ...
      try {
        ...
        const enrichedProfiles = profiles.map((p) => {
          const daysUntilExpiration = p.membership_expiration
            ? Math.ceil(
                (new Date(p.membership_expiration).getTime() - now.getTime()) /
                  (1000 * 60 * 60 * 24)
              )
            : null;
        ...
        const totalLTV = enrichedProfiles.reduce(
          (sum, p) => sum + (Number(p.lifetime_value) || 0),
          0
        );
        ...
        const marketingOptIn: MarketingOptIn = {
          ...
          percentage:
            membersWithMarketingData.length > 0
