          ...
          <div className="flex items-start justify-between">
            <div className="flex flex-wrap gap-1.5">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-block px-2.5 py-1 rounded-md bg-black/40 backdrop-blur-md border border-white/20 text-[10px] font-bold text-white uppercase tracking-wider"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-6">
          <div className="col-span-1 lg:col-span-2 flex flex-col justify-between rounded-[32px] border border-slate-100 p-8 md:p-10 bg-white shadow-[0_4px_24px_-12px_rgba(0,0,0,0.05)] min-h-[300px] lg:min-h-full">
            <h3 className="text-2xl md:text-4xl font-bold tracking-tight text-slate-900 leading-[1.15] mb-8">
              Every home is different. <br className="hidden lg:block" />
              Kandie Gang works in yours.
            </h3>
            <p className="text-slate-500 font-light leading-relaxed text-sm md:text-base">
              We believe Kandie Gang should be easy to use in the real-world, working autonomously
              out-of-the-box. Hundreds of people in unique homes show Kandie Gang how chores are
              done each day, so Kandie Gang can robustly handle the chaos of real life.
            </p>
          </div>
