...
const SPECIFICATIONS = [
  {
    term: 'Kandie Code',
    value: 'Kandie Kodex — unser gemeinsames Verhalten in der Kandie Gang',
    href: 'kandie-code' as SpecHref,
  },
  {
    term: 'Ride Levels',
    value: 'Eckdaten zu den vier Kandie-Gruppen — finde Dein passendes Level',
    href: 'ride-levels' as SpecHref,
  },
  {
    term: 'Ride Waiver',
    value: 'Haftungsausschluss für gemeinsame sportliche Aktivitäten der Kandie Gang',
    href: 'ride-waiver' as SpecHref,
  },
] as const;
    ...
    >
      ...
      <div className="col-span-full lg:col-span-6 lg:col-start-1">
        <h2
          ...
          className="font-heading font-gtplanar text-h4 text-secondary-current text-balance"
        >
      ...
      <div className="col-span-full mt-10 lg:mt-12">
        ...
        <KandieCodeModal
          isOpen={kandieCodeModalOpen}
          onClose={() => {
            setKandieCodeModalOpen(false);
            setDone({ kandieCode: true });
          }}
        />
        <RideLevelsModal
          isOpen={rideLevelsModalOpen}
          onClose={() => {
            setRideLevelsModalOpen(false);
            setDone({ rideLevels: true });
          }}
        />
        <WaiverModal
          isOpen={waiverModalOpen}
          onClose={() => setWaiverModalOpen(false)}
          onSignWaiver={() => setDone({ waiver: true })}
        />
      </div>
    </section>
  );
