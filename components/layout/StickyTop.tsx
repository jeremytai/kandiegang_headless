...
export const StickyTop: React.FC<StickyTopProps> = ({ offsetVariant = 'withBar' }) => {
  ...
  const showMemberAreaLink = isLoggedIn && (isCyclingMember(profile?.membership_plans) || isGuide);
          ...
          <div className="grid grid-cols-3 items-center px-2.5 py-2.5">
            <div className="flex items-center gap-1 pl-3">
              {isContactPage ? (
                <Link
                  to="/"
                  className="flex items-center gap-2 font-bold text-slate-900 tracking-tighter text-xs"
                >
              ...
              {!isContactPage && (
                ...
                >
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {isExpanded ? (
                        <X className="w-4 h-4 text-secondary-purple-rain group-hover:text-white" />
                      ) : (
                        <Menu className="w-4 h-4 text-secondary-purple-rain group-hover:text-white" />
                      )}
                    </motion.div>
                  </AnimatePresence>
                </button>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    <nav className="flex flex-col gap-2">
                      <NavLink label="Ride Calendar" to="/community" onClick={handleToggle} />
                      {showMemberAreaLink ? (
                      ...
                      <NavLink label="About Us" to="/about" onClick={handleToggle} />
                    </nav>
                  </div>
