...
const EventHeader: React.FC<EventHeaderProps> = ({ title, intro, imageUrl, onBack }) => {
  return (
    ...
    <section className="bg-[#f3f4f6] py-12 md:py-20 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="order-2 lg:order-1 lg:col-span-7">
            {onBack && (
              <button 
                onClick={onBack} 
                className="flex items-center gap-2 text-sm font-medium uppercase tracking-wider text-gray-600 hover:text-black transition-colors mb-6"
              >
            ...
            <h1 className="text-4xl md:text-6xl font-bold text-[#2a0d5d] leading-tight mb-6">
              {title}
            </h1>
            
            {intro && (
              <div className="max-w-md">
                <p className="text-xl md:text-2xl font-semibold leading-snug text-gray-900">
                  {intro}
                </p>
              </div>
            )}
