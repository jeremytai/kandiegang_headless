  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`w-full bg-[#f2f2f2]/60 backdrop-blur-2xl border border-white/20 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.15)] overflow-hidden pointer-events-auto ${
          isExpanded ? 'max-w-[640px] rounded-[32px]' : 'max-w-[340px] rounded-full'
        }`}
      >
        <div className="grid grid-cols-3 items-center px-2.5 py-2.5">
          ...
          <div className="flex justify-center">
            {!isContactPage && (
              <Link to="/" className="flex items-center gap-1.5 hover:opacity-70 transition-opacity">
                <img 
                  src="/logos/kandiegang_horiz_log.svg" 
                  alt="Kandie Gang" 
...
const NavLink = ({ label, to, onClick }: { label: string, to: string, onClick: () => void }) => (
  <Link 
    to={to} 
    onClick={onClick}
    className="text-2xl font-bold text-slate-900 hover:translate-x-2 transition-transform duration-300 block tracking-tight"
  >
    {label}
  </Link>
);