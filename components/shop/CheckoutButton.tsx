...
export const CheckoutButton: React.FC<CheckoutButtonProps> = ({
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!text) {
        throw new Error(
          'Empty response from server. In development, use `vercel dev` instead of `npm run dev` to enable API routes. Or deploy to Vercel to test checkout.'
        );
      }
      ...
      try {
        if (contentType?.includes('application/json')) {
          ...
          throw new Error(
            `Unexpected response type: ${contentType || 'unknown'}. Response: ${text.substring(0, 100)}`
          );
        }
      } catch (error) {
