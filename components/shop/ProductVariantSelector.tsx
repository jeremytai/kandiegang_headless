      ...
      <div className="flex flex-col gap-2 w-full max-w-xs">
        <div className="flex flex-col gap-0.5">
          {showVariantRequiredMessage ? (
            ...
            <label
              ...
              className="text-sm font-medium text-secondary-purple-rain/70 uppercase tracking-widest"
            >
              {variantLabelProp}
            </label>
          )}
        ...
        >
          {variants.map((v, index) => (
            <option key={index} value={index} disabled={v.inventory <= 0}>
              {v.inventory <= 0
                ? `${formatVariantLabel(v.label)} (Out of stock)`
