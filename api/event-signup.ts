...
async function fetchEventAccessData(eventId: number): Promise<EventAccessData | null> {
  const query = `
    query GetRideEventAccess($id: ID!) {
      rideEvent(id: $id, idType: DATABASE_ID) {
        publicReleaseDate
        eventDetails {
          isFlintaOnly
          workshopCapacity
          level1 { guides { nodes { id } } }
          level2 { guides { nodes { id } } }
          level2plus { guides { nodes { id } } }
          level3 { guides { nodes { id } } }
        }
      }
    }
  `;
...
<body style="font-family: system-ui, -apple-system, sans-serif; line-height: 1.6; color: #1F2223; max-width: 560px; margin: 0 auto; padding: 24px;">
  <h1 style="font-size: 1.4rem; color: #46519C; font-weight: 600;">Your event spot is saved</h1>
  <p>We have you on the list for:</p>
  ...
  <p style="margin-top: 8px;">
    <a href="{{CANCEL_URL}}" style="color: #46519C;">Cancel my spot</a>
  </p>
</body>
</html>`;
}
...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="font-family: system-ui, -apple-system, sans-serif; line-height: 1.6; color: #1F2223; max-width: 560px; margin: 0 auto; padding: 24px;">
  <h1 style="font-size: 1.4rem; color: #46519C; font-weight: 600;">You are on the waitlist</h1>
  <p>We added you to the waitlist for:</p>
  <p><strong>${eventTitle}</strong><br/>${rideLevel}</p>
  <p style="margin-top: 24px; font-size: 0.9rem; color: #5f6264;">If a spot opens, we will email you right away.</p>
  <p style="margin-top: 8px;">
    <a href="{{CANCEL_URL}}" style="color: #46519C;">Leave the waitlist</a>
  </p>
</body>
</html>`;
}
