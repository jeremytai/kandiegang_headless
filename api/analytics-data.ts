...
async function handleEventParticipation(res: NextApiResponse, adminClient: any) {
  try {
    ...
    const events = eventIds.map((eid) => {
      ...
      let confirmed = 0, waitlist = 0, cancelled = 0;
      ...
      const registrants = group.rows.map((row: any) => {
        ...
        if (isCancelled) { byLevel[level].cancelled++; cancelled++; }
        else if (isWaitlist) { byLevel[level].waitlist++; waitlist++; }
      ...
      // Sort levels canonically
      const sortedByLevel: typeof byLevel = {};
      [...LEVEL_ORDER, ...Object.keys(byLevel).filter((l) => !LEVEL_ORDER.includes(l))].forEach(
        (l) => { if (byLevel[l]) sortedByLevel[l] = byLevel[l]; }
      );

      return { eventId: eid, title: meta.title, date: meta.date, eventType: group.eventType, confirmed, waitlist, cancelled, byLevel: sortedByLevel, registrants };
    });
